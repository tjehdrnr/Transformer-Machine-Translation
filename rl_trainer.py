...
class MinimumRiskTrainingEngine(MaximumLikelihoodEstimationEngine):
    ...
    def _get_reward(y_hat, y, n_gram=6, method='gleu'):
        # y_hat and y are one-hot index tensor.
        ...
        score_func = {
            ...
            'bleu': lambda ref, hyp: sentence_bleu([ref], hyp, 
        ...
        # Since we don't calculate reward scroe exactly as same as multi-bleu.perl,
        ...
        with torch.no_grad():
            ...
            for samp in range(y.size(0)):
                ref, hyp = [], []
                for t in range(y.size(-1)):
                    ref.append(str(int(y[samp, t])))
                    if y[samp, t] == dataloader.EOS:
                        break
                
                for t in range(y_hat.size(-1)):
                    hyp.append(str(int(y_hat[samp, t])))
                    if y_hat[samp, t] == dataloader.EOS:
                        break
                
                scores.append(score_func(ref, hyp) * 100.)
    ...
    def _get_loss(y_hat, indice, reward=1):
        # |indice| = (batch_size, length) --> one-hot index
        # |y_hat| = (batch_size, length, output_size) --> log-probability
        # |reward| = (batch_size,)
    ...
    def train(engine, mini_batch):
        ...
        y_hat, indice = engine.model.search(
            x,
            is_greedy=False, # Use multinoulli distribution sampling.
    ...
    def validate(engine, mini_batch):
        ...
        with torch.no_grad():
            device = next(engine.model.parameters()).device
            mini_batch.src = (mini_batch.src[0].to(device), mini_batch.src[1])
            mini_batch_tgt = (mini_batch.tgt[0].to(device), mini_batch.tgt[1])
    ...
    def attach(
        ...
        if verbose >= VERBOSE_EPOCH_WISE:
            ...
            def print_train_logs(engine):
                ...
                print('Epoch {} - |param|={:.2e}  |g_param|={:.2e}  BLEU={:.2e}'.format(
                    engine.state.epoch,
                    avg_p_norm,
                    avg_g_norm,
                    avg_reward
    ...
    @staticmethod
    def check_best(engine, train_engine, config, src_vocab, tgt_vocab):
        loss = -float(engine.state.metrics['BLEU'])
        if loss <= engine.best_loss:
    ...
    def save_model(engine, train_engine, config, src_vocab, tgt_vocab):
        ...
        # Set a filname for model of last epoch.
