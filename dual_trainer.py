...
class DualSupervisedTrainingEngine(Engine):
    ...
    def _get_loss(x, y, x_hat, y_hat, crits, x_lm=None, y_lm=None, lagrange=1e-3):
        ...
        # |log_p_y_given_x| = (batch_size * n)
        # |log_p_x_given_y| = (batch_size * m)
        ...
        if x_lm is not None and y_lm is not None:
            ...
            dual_loss = lagrange * ((log_p_x + log_p_y_given_x.detach()) -
                                    (log_p_y + log_p_x_given_y.detach()))**2
            
            # Note that 'detach()' is used to prevent unnecessary back-propagation.
            loss_x2y += lagrange * ((log_p_x + log_p_y_given_x) -
                                    (log_p_y + log_p_x_given_y.detach()))**2
            loss_y2x += lagrange * ((log_p_x + log_p_y_given_x.detach()) -
    ...
    def train(engine, mini_batch):
        ...
        with autocast(not engine.config.off_autocast):
            ...
            loss_x2y, loss_y2x, dual_loss = DualSupervisedTrainingEngine._get_loss(
                ...
                # Thus, we turn-off the regularization at the begining.
    ...
    def attach(
        ...
        if verbose >= VERBOSE_EPOCH_WISE:
            ...
            def print_train_logs(engine):
                ...
                print('Epoch {} - |param|={:.2e}  |g_param|={:.2e}  loss_x2y={:.4f} ppl_x2y={:.2f}  \
                loss_y2x={:.4f} ppl_x2y={:.2f}  dual_loss={:.4e}'.format(
                    engine.state.epoch,
                    avg_p_norm,
                    avg_g_norm,
        ...
        if verbose >= VERBOSE_EPOCH_WISE:
            ...
            def print_valid_logs(engine):
                ...
                print('Validation X2Y - loss={:.4e} ppl={:.2f}  best_loss={:.4e}  \
                best_ppl={:2f}'.format(
                    avg_x2y,
                    np.exp(avg_x2y),
                    engine.best_x2y,
                ...
                print('Validation Y2X - loss={:.4e} ppl={:.2f}  best_loss={:.4e}  \
                best_ppl={:2f}'.format(
                    avg_y2x,
                    np.exp(avg_y2x),
                    engine.best_y2x,
    ...
    def check_best(engine):
        ...
        if x2y <= engine.best_x2y:
            print('Best X2Y loss has changed from {:.4e} to {:.4e}'.format(
              engine.best_x2y, x2y))
            engine.best_x2y = x2y  
        y2x = float(engine.state.metrics['y2x'])
        if y2x <= engine.best_y2x:
            print('Best Y2X loss has changed from {:.4e} to {:.4e}'.format(
              engine.best_y2x, y2x))
            engine.best_y2x = y2x
    ...
    def save_model(engine, train_engine, config, vocabs):
        ...
        # model_fn = train_engine.state.epoch + '_' + config.model_fn
