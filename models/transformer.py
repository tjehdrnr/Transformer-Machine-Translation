...
class Attention(nn.Module):
    ...
    def forward(self, Q, K, V, mask=None, d_head=64):
        # |Q| = (batch_size * n_heads, m, hidden_size / n_heads)
        # |K|, |V| = (batch_size * n_heads, n, hidden_size / n_heads)
        # |mask| = (batch_size * n_heads, m, n)

        scores = torch.bmm(Q, K.transpose(1, 2))
        # |scores| = (batch_size * n_heads, m, n)

        if mask is not None:
            assert scores.size() == mask.size()
            scores.masked_fill_(mask, -float('inf'))
        
        weights = self.softmax(scores / (d_head**.5))
        # |weights| = (batch_size * n_heads, m, n)
        context_vector = torch.bmm(weights, V)
        # |context_vector| = (batch_size * n_heads, m, hidden_size / n_heads)
...
class MultiHead(nn.Module):
    ...
    def forward(self, Q, K, V, mask=None):
        ...
        QWs = self.Q_linear(Q).split(self.hidden_size // self.n_heads, dim=-1)
        KWs = self.K_linear(K).split(self.hidden_size // self.n_heads, dim=-1)
        VWs = self.V_linear(V).split(self.hidden_size // self.n_heads, dim=-1)
        # |QW_i| = (batch_size, m, hidden_size / n_heads)
        # |KW_i|, |VW_i| = (batch_size, n, hidden_size / n_heads)
        ...
        # |QWs| = (batch_size * n_heads, m, hidden_size / n_heads)
        # |KWs|, |VWs| = (batch_size * n_heads, n, hidden_size / n_heads)

        if mask is not None:
            mask = torch.cat([mask for _ in range(self.n_heads)], dim=0)
            # |mask| = (batch_size * n_heads, m, n)
        
        context_vector = self.attn(
            QWs, KWs, VWs,
            mask=mask,
            d_head=self.hidden_size // self.n_heads
        )
        # |context_vector| = (batch_size * n_heads, m, hidden_size / n_heads)

        # We need to restore temporal mini_batchfied multi-head attention results.
        context_vector = context_vector.split(Q.size(0), dim=0)
        # |context_vector_i| = (batch_size, m, hidden_size / n_heads)
        context_vector = self.linear(torch.cat(context_vector, dim=-1))
        # |context_vector| = (batch_size, m, hidden_size)
...
class EncoderBlock(nn.Module):
    ...
    def __init__(
        ...
        self.attn = MultiHead(hidden_size, n_heads)
    ...
    def forward(self, x, mask):
        ...
        # z = self.attn_norm(x + self.attn_dropout(self.attn(
...
class DecoderBlock(nn.Module):
    ...
    def __init__(
        ...
        self.masked_attn = MultiHead(hidden_size, n_heads)
    ...
    def forward(self, x, key_and_value, mask, prev, future_mask):
        ...
        if prev is None: # Training mode
            ...
            # |prev| = (batch_size, |0:t|, hidden_size)
            # |future_mask| = None
        ...
        # Post-LN:
        # z = self.attn_norm(z + self.attn_dropout(
        #     self.attn(Q=z, K=key_and_value, V=key_and_value, mask=mask)
        # ))
...
class Transformer(nn.Module):
    ...
    def _generate_pos_enc(self, hidden_size, max_length):
        ...
        enc[:, 0::2] = torch.sin(pos / 1e+4**dim.div(float(hidden_size))) # even
        enc[:, 1::2] = torch.cos(pos / 1e+4**dim.div(float(hidden_size))) # odd
    ...
    def search(self, x, is_greedy=True, max_length=255):
        ...
        y_t_1 = x.new(batch_size, 1).zero_() + dataloader.BOS
        # |y_t_1| = (batch_size, 1)
        ...
        while is_decoding.sum() > 0 and len(indice) < max_length:
            ...
            for layer_index, block in enumerate(self.decoder._modules.values()):
                ...
                if prevs[layer_index + 1] is None:
                    ...
                    prevs[layer_index + 1] = torch.cat(
                        [prevs[layer_index + 1], h_t], dim=1
                    )
                # |prevs| = (batch_size, len(y_hats) + 1, hidden_size)
            ...
            if is_greedy: # Argmax
                y_t_1 = torch.topk(y_hat_t, k=1, dim=-1)[1].squeeze(-1)
            ...
            is_decoding = is_decoding * torch.ne(y_t_1, dataloader.EOS)
            # |y_t_1| = (batch_size, 1)
    ...
    def batch_beam_search(
        ...
        while sum(done_cnt) < batch_size and length <= max_length:
            ...
            for i, board in enumerate(boards): # i == sample_index in mini-batch
                if board.is_done() == 0:
                    ...
                    for layer_index in range(n_dec_layers + 1):
                        ...
                        if prev_i is not None:
                            fab_prevs[layer_index].append(prev_i)
                        else:
            ...
            # |fab_input|    = (batch_size * beam_size, 1,)
            # |fab_z|        = (batch_size * beam_size, n, hidden_size)
            # |fab_mask|     = (batch_size * beam_size, 1, n)
            # |fab_prevs[i]| = (batch_size * beam_size, length, hidden_size)
            # len(fab_prevs) = n_dec_layers + 1

            # Unlike training procedure,
            # take the lask time-step's output during the inference.
            h_t = self.emb_dropout(
                self._position_encoding(self.emb_dec(fab_input), init_pos=length)
            )
            # |h_t| = (batch_size * beam_size, 1, hidden_size)
            ...
            for layer_index, block in enumerate(self.decoder._modules.values()):
                ...
                # |prev| = (batch_size * beam_size, 1, hidden_size)
            ...
            for board in boards:
                if board.is_done() == 0:
                    ...
                    prev_status = {}
                    for layer_index in range(n_dec_layers + 1):
                        prev_status['prev_state_%d' % layer_index] = fab_prevs[
                            layer_index][begin:end]
